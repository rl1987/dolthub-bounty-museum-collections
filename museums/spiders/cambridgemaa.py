...
class CambridgemaaSpider(scrapy.Spider):
    ...
    def start_requests(self):
        query_dict = {"string": "", "options": [], "currentPage": 1, "filters": []}

        params = {"query": json.dumps(query_dict)}

        url1 = "https://collections.maa.cam.ac.uk/objects-api/objects/?" + urlencode(
            params
        )
        ...
        url2 = (
            "https://collections.maa.cam.ac.uk/photographs-api/photographs/?"
            + urlencode(params)
        )
        yield scrapy.Request(
            url2,
    ...
    def parse_object_page(self, response):
        ...
        item["materials"] = "|".join(
            list(map(lambda m: m.get("name"), payload.get("material", [])))
        )
        ...
        item["culture"] = "|".join(
            list(map(lambda c: c.get("name"), payload.get("culture_group", [])))
        )
        if payload.get("date") is not None:
            item["date_description"] = payload.get("date")
        else:
            item["date_description"] = "; ".join(
                list(map(lambda p: p.get("name"), payload.get("period")))
            )
        ...
        if (
            ...
            and payload.get("collections_accession") is not None
        ):
            item["accession_number"] = payload.get("collections_accession", dict()).get(
                "number"
            )
        if payload.get("main_image") is not None:
            item["image_url"] = payload.get("main_image", dict()).get("image")
        item["source_1"] = "https://collections.maa.cam.ac.uk/objects/" + str(
            payload.get("id")
        )
