...
class IndianaSpider(scrapy.Spider):
    ...
    start_urls = ['https://collection.indianamuseum.org/mwebcgi/mweb.exe?request=record;id={};type=101'.format(i) for i in range(1, 1000000)]
    ...
    def parse(self, response):
        ...
        item['object_number'] = response.xpath('//dt[text()="ID Number"]/following-sibling::dd/text()').get()
        ...
        # XXX: culture
        item['date_description'] = response.xpath('//dt[text()="Year"]/following-sibling::dd/text()').get()
        item['maker_full_name'] = "|".join(response.xpath('//dt[text()="Maker"]/following-sibling::dd//a/text()').getall())
        # XXX: acquired, accession...
        item['credit_line'] = response.xpath('//dt[text()="Credit Line"]/following-sibling::dd/text()').get()
        item['image_url'] = response.xpath('//div[@class="fullRecImage"]//img/@src').get()
