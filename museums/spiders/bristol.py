...
class BristolSpider(scrapy.Spider):
    ...
    def parse_object_page(self, response):
        ...
        item['object_number'] = response.xpath('//p[./label[text()="Object Number"]]/text()').get()
        ...
        item['department'] = response.xpath('//p[./label[text()="Collection"]]/text()').get()
        item['title'] = response.xpath('//div[@class="page-header"]/h3/text()').get("").strip()
        item['description'] = response.xpath('//p[./label[text()="Description"]]/text()').get()
        item['current_location'] = " ".join(response.xpath('//div[./div/h3[text()="Current Location"]]/text()').getall()).strip()
        item['maker_full_name'] = response.xpath('//p[./label[text()="Artist"]]/text()').get()
