...
class NpglondonSpider(scrapy.Spider):
    ...
    start_urls = [
        "https://www.npg.org.uk/collections/search/arta-z/?index=" + letter
        for letter in string.ascii_lowercase
    ] + [
        "https://www.npg.org.uk/collections/search/sita-z/?index=" + letter
        for letter in string.ascii_lowercase
    ]
    ...
    def parse_portrait_page(self, response):
        ...
        item["title"] = (
            ...
            .get()
            .split("; ")[-1]
            .replace(" - National Portrait Gallery", "")
        )
        item["description"] = response.xpath(
            '//meta[@property="og:description"]/@content'
        ).get()
        item["maker_full_name"] = "|".join(
            response.xpath('//ul[@id="artist-list"]//a/text()').getall()
        )

        lines = response.xpath(
            '//div[@class="contentColumn last"]/p[last()]/text()'
        ).getall()
