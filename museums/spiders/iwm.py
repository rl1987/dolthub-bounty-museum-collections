...
class IwmSpider(scrapy.Spider):
    ...
    start_urls = [
        "https://www.iwm.org.uk/collections/search?query=&pageSize=30&media-records=all-records&page=1"
    ]
    ...
    def parse_search_page(self, response):
        links = response.xpath(
            '//div[@class="teaser-list__title--content"]/a/@href'
        ).getall()
        for l in links:
    ...
    def parse_object_page(self, response):
        ...
        item["object_number"] = response.xpath(
            '//dt[text()="Catalogue number"]/following-sibling::dd/text()'
        ).get()
        ...
        item["category"] = response.xpath(
            '//dt[text()="Category"]/following-sibling::dd/a/text()'
        ).get()
        item["title"] = (
            response.xpath('//h1[@class="hero__truncate"]/text()').get("").strip()
        )
        item["description"] = response.xpath(
            '//div[@class="object-description__text"]/text()'
        ).get()
        item["dimensions"] = "|".join(
            response.xpath(
                '//dt[text()="Dimensions"]/following-sibling::dd/p/text()'
            ).getall()
        )
        item["materials"] = "|".join(
            response.xpath(
                '//dt[text()="Materials"]/following-sibling::dd/p/text()'
            ).getall()
        )
        item["from_location"] = response.xpath(
            '//dt[text()="Place made"]/following-sibling::dd/a/text()'
        ).get()
        item["date_description"] = response.xpath(
            '//dt[text()="Production date"]/following-sibling::dd/text()'
        ).get()
        item["maker_full_name"] = "|".join(
            response.xpath(
                '//dt[text()="Creator"]/following-sibling::dd/a/text()'
            ).getall()
        )
