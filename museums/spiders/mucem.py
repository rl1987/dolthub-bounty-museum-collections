...
class MucemSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for i in range(1, 500000):
            ...
            url = (
                "https://www.mucem.org/en/collections/explorez-les-collections/objet?uri="
                + data_url
            )
    ...
    def parse_object_page(self, response):
        ...
        item["object_number"] = response.xpath(
            '//div[./label[text()="Numero d\'inventaire :"]]/div[@class="detail_objet"]/p/text()'
        ).get()
        ...
        item["description"] = " ".join(
            response.xpath('//div[./label[text()="Description :"]]//p/text()').getall()
        )
        item["dimensions"] = "|".join(
            response.xpath(
                '//div[./label[text()="Dimensions et poids :"]]//p/text()'
            ).getall()
        )
        item["materials"] = "|".join(
            response.xpath(
                '//div[./label[text()="Materials & techniques :"]]//p/text()'
            ).getall()
        )
        item["from_location"] = response.xpath(
            '//label[@class="lieu" and text()="Emplacement :"]/following-sibling::div/p/text()'
        ).get()
        item["date_description"] = response.xpath(
            '//label[@class="date" and text()="Date :"]/following-sibling::div/p/text()'
        ).get()
        item["maker_full_name"] = (
            response.xpath(
                '//label[@class="prenom" and text()="Author / Performer :"]/following-sibling::div/p/span/text()'
            )
            .get("")
            .strip()
        )
