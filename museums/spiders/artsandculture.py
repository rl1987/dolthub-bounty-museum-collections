...
class ArtsAndCultureSpider(scrapy.Spider):
    ...
    def partner_api_request_from_pt(self, pt):
        params = {"s": 24, "pt": pt, "hl": "en", "rt": "j"}

        url = "https://artsandculture.google.com/api/objects/partner?" + urlencode(
            params
        )
    ...
    def parse_asset_search_html_page(self, response):
        ...
        search_links = parsed.xpath(
            '//string[starts-with(text(), "/search/asset")]/text()'
        )
    ...
    def parse_asset_html_page(self, response):
        ...
        item["object_number"] = (
            response.xpath('//li[./span[text()="Number:"]]/text()').get("").strip()
        )
        if item["object_number"] == "":
            item["object_number"] = response.xpath(
                '//div[@jsmodel="lLrbSb"]/@data-prid'
            ).get()
        item["institution_name"] = response.xpath('//h3[@class="To7WBf"]/text()').get()

        institution_city_country = response.xpath(
            '//span[@class="WrfKPd"]/text()'
        ).get()
        ...
        item["title"] = (
            response.xpath('//li[./span[text()="Title:"]]/text()').get("").strip()
        )
        item["dimensions"] = (
            response.xpath('//li[./span[text()="Physical Dimensions:"]]/text()')
            .get("")
            .strip()
        )
        item["inscription"] = (
            response.xpath('//li[./span[text()="Inscription:"]]/text()').get("").strip()
        )
        item["provenance"] = (
            response.xpath('//li[./span[text()="Provenance:"]]/text()').get("").strip()
        )
        item["materials"] = " ".join(
            response.xpath('//li[./span[text()="Medium:"]]/*/text()').getall()
        ).replace("Medium: ", "")
        item["technique"] = (
            response.xpath('//li[./span[text()="Technique:"]]/text()').get("").strip()
        )
        item["from_location"] = (
            response.xpath('//li[./span[text()="Location Created:"]]/text()')
            .get("")
            .strip()
        )
        if item["from_location"] == "":
            item["from_location"] = (
                response.xpath('//li[./span[text()="Location:"]]/text()')
                .get("")
                .strip()
            )

        item["date_description"] = (
            response.xpath('//li[./span[text()="Date:"]]/text()').get("").strip()
        )

        item["source_1"] = response.url
        item["source_2"] = response.xpath(
            '//li[./span[text()="External Link:"]]/a/@href'
        ).get()
