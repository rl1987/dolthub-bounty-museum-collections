...
class LouvreSpider(scrapy.Spider):
    ...
    def _get_maker_birth_year(self, creator_dict):
        for date_dict in creator_dict.get("dates", []):
            if (
                date_dict.get("type") == "date 1"
                or date_dict.get("type") == "date de naissance"
            ):
    ...
    def _get_maker_death_year(self, creator_dict):
        for date_dict in creator_dict.get("dates", []):
            if (
                date_dict.get("type") == "date 2"
                or date_dict.get("type") == "date de mort"
            ):
                return date_dict.get("date")
    ...
    def parse_search_page(self, response):
        ...
        if (
            response.xpath(
                '//button[@aria-label="Show next page" and not(contains(@class, "inactive"))]'
            )
            is None
        ):
            return
    ...
    def parse_artwork_page(self, response):
        ...
        item["object_number"] = "".join(
            response.xpath(
                '//div[./div[text()="Inventory number"]]/div[last()]/text()'
            ).getall()
        ).strip()
        ...
        item["department"] = response.xpath(
            '//div[./div[text()="Collection"]]/div[last()]/a/text()'
        ).get()
        item["category"] = response.xpath(
            '//div[./div[text()="Category"]]/div[last()]/a/text()'
        ).get()
        item["title"] = response.xpath('//h1[@class="notice__title h_1"]/text()').get()
        item["description"] = "".join(
            response.xpath(
                '//div[./h2[text()="Description"]]//div[./div[text()="Description/Features"]]/div[last()]/text()'
            ).getall()
        ).strip()
        item["current_location"] = " ".join(
            response.xpath(
                '//div[./div[text()="Current location"]]/div[last()]/div/text()'
            ).getall()
        ).strip()
        try:
            item["dimensions"] = (
                response.xpath('//div[./div[text()="Dimensions"]]/div[last()]/text()')
                .get()
                .strip()
            )
        except:
        ...
        item["inscription"] = " ".join(
            response.xpath(
                '//div[./div[text()="Inscriptions"]]/div[last()]/text()'
            ).getall()
        ).strip()
        item["provenance"] = " ".join(
            response.xpath(
                '//div[./div[text()="Object history"]]/div[last()]/text()'
            ).getall()
        ).strip()  # XXX: is this correct?

        item["materials"] = " - ".join(
            response.xpath(
                '//div[./div[text()="Materials"]]/div[last()]/a/text()'
            ).getall()
        ).strip()
        item["technique"] = " - ".join(
            response.xpath(
                '//div[./div[text()="Techniques"]]/div[last()]/a/text()'
            ).getall()
        ).strip()
        # XXX: from_location
        # XXX: culture
        item["date_description"] = response.xpath(
            '//div[./div[text()="Date"]]/div[last()]/text()'
        ).get()

        item["maker_full_name"] = response.xpath(
            '//div[./div[text()="Artist/maker / School / Artistic centre"]]/div[last()]/a[1]/text()'
        ).get()
        ...
        try:
            item["acquired_from"] = (
                response.xpath(
                    '//div[./div[text()="Collector / Previous owner / Commissioner / Archaeologist / Dedicatee"]]/div[last()]/text()'
                )
                .get()
                .strip()
            )  # XXX: sometimes there's links in this
        except:
            pass
        ...
        item["source_2"] = response.xpath(
            '//span[contains(text(), "JSON Record")]/a/@href'
        ).get()  # TODO: consider also parsing the JSON file

        yield scrapy.Request(
            item["source_2"], callback=self.parse_artwork_json, meta={"item": item}
        )
    ...
    def parse_artwork_json(self, response):
        ...
        if (
            json_dict.get("acquisitionDetails") is not None
            and len(json_dict.get("acquisitionDetails")) > 0
        ):
            try:
                item["acquired_year"] = (
                    json_dict.get("acquisitionDetails")[-1]
                    .get("dates")[-1]
                    .get("startYear")
                )
            except:
        ...
        if creators is not None and len(creators) > 0:
            ...
            maker_birth_years = list(
                map(lambda c: self._get_maker_birth_year(c), creators)
            )
            maker_birth_years = "|".join(maker_birth_years)
            item["maker_birth_year"] = maker_birth_years

            maker_death_years = list(
                map(lambda c: self._get_maker_death_year(c), creators)
            )
