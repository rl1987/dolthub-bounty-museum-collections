...
class SeamuseumSpider(scrapy.Spider):
    ...
    start_urls = [
        "http://collections.sea.museum/search/*/objects?filter=MediaExistence%3Atrue#filters",
    ...
    def parse_search_page(self, response):
        for object_link in response.xpath(
            '//a[starts-with(@href, "/objects/")]/@href'
        ).getall():
    ...
    def parse_object_page(self, response):
        ...
        item["object_number"] = response.xpath(
            '//div[contains(@class, "invnoField")]/span[@class="detailFieldValue"]/text()'
        ).get()
        ...
        item["title"] = response.xpath(
            '//div[contains(@class, "titleField")]/h1/text()'
        ).get()
        item["category"] = response.xpath(
            '//div[contains(@class, "classificationField")]/span/a/text()'
        ).get()
        item["description"] = " ".join(
            response.xpath('//div[@class="descriptionText"]/text()').getall()
        )
        item["dimensions"] = response.xpath(
            '//div[contains(@class, "dimensionsField")]/span[@class="detailFieldValue"]/div/text()'
        ).get()
        ...
        item["from_location"] = response.xpath(
            '//div[./span[text()="Place Manufactured:"]]/span[last()]/text()'
        ).get()
        item["date_description"] = response.xpath(
            '//div[contains(@class, "displayDateField")]/span[@class="detailFieldValue"]/text()'
        ).get()
        item["maker_full_name"] = response.xpath(
            '//div[contains(@class, "primaryMakerField")]/span/a/text()'
        ).get()
