...
class PennSpider(scrapy.Spider):
    ...
    start_urls = [
        "https://www.penn.museum/collections/search.php?term=&submit_term=Submit+Query&type%5B%5D=1"
    ]
    ...
    def parse_search_results(self, response):
        for object_link in response.xpath(
            '//a[./img[contains(@class, "center-block")]]/@href'
        ).getall():
    ...
    def parse_object_page(self, response):
        ...
        item["department"] = response.xpath(
            '//tr[./th[text()="Section:"]]/td/text()'
        ).get()
        item["title"] = response.xpath('//h1[@itemprop="name"]/text()').get("").strip()
        item["description"] = " ".join(
            response.xpath('//div[@itemprop="description"]/*/text()').getall()
        )
        item["current_location"] = (
            response.xpath('//tr[./th[contains(text(),"Current Location:")]]/td/text()')
            .get("")
            .strip()
        )
        if item["current_location"] == "":
            item["current_location"] = (
                response.xpath(
                    '//tr[./th[contains(text(),"Current Location:")]]/td/a/text()'
                )
                .get("")
                .strip()
            )
        ...
        outside_diameter = response.xpath(
            '//tr[./th[text()="Outside Diameter:"]]/td/text()'
        ).get()
        if outside_diameter is not None:
            item["dimensions"] += "Outside Diameter: " + outside_diameter + " "
        item["provenance"] = " ".join(
            response.xpath('//tr[./th[text()="Provenience: "]]/td/text()').getall()
        )
        item["materials"] = " ".join(
            response.xpath('//tr[./th[text()="Materials:"]]/td/text()').getall()
        )
        item["technique"] = response.xpath(
            '//tr[./th[text()="Technique:"]]/td/text()'
        ).get()
        item["from_location"] = response.xpath(
            '//tr[./th[text()="Locus: "]]/td/text()'
        ).get()
        item["culture"] = response.xpath(
            '//tr[./th[text()="Culture Area: "]]/td/text()'
        ).get()
        item["date_description"] = response.xpath(
            '//tr[./th[text()="Date Made: "]]/td/text()'
        ).get()
        item["year_start"] = response.xpath(
            '//tr[./th[text()="Early Date: "]]/td/text()'
        ).get()
        item["year_end"] = response.xpath(
            '//tr[./th[text()="Late Date: "]]/td/text()'
        ).get()
        item["maker_full_name"] = response.xpath(
            '//tr[./th[text()="Maker: "]]/td/text()'
        ).get()
        item["credit_line"] = response.xpath(
            '//tr[./th[text()="Credit Line:"]]/td/a/text()'
        ).get()
