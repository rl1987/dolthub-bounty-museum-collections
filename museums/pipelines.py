...
class ConstraintEnforcementPipeline:
    def process_item(self, item, spider):
        ...
        if object_number is None or len(object_number) == 0:
            raise DropItem("Constraint violation on item {}: empty object_number".format(object_number))

        institution_name = adapter.get("institution_name")
        if institution_name is None or len(institution_name) == 0:
            raise DropItem("Constraint violation on item {}: empty institution_name".format(institution_name))
        ...
        if institution_latitude is not None and type(institution_latitude) != float:
            try:
                ...
                raise DropItem("Constraint violation on item {}: institution_latitude cannot be converted to float".format(item))
        ...
        if institution_longitude is not None and type(institution_longitude) != float:
            try:
                ...
                raise DropItem("Constraint violation on item {}: institution_longitude cannot be converted to float".format(item))
        ...
        if year_start is not None and type(year_start) != int:
            try:
                ...
                raise DropItem("Constraint violation on item {}: year_start cannot be converted to int".format(item))
        ...
        if year_end is not None and type(year_end) != int:
            try:
                ...
                raise DropItem("Constraint violation on item {}: year_end cannot be converted to int".format(item))
        ...
        if acquired_year is not None and type(acquired_year) != int:
            try:
                ...
                raise DropItem("Constraint violation on item {}: acquired_year cannot be converted to int".format(acquired_year))
        ...
        if accession_year is not None and type(accession_year) != int:
            try:
                ...
                raise DropItem("Constraint violation on item {}: accession_year cannot be converted to int".format(item))
        ...
        if type(image_url) == str and len(image_url) > 2000: # XXX: maybe implement smarter truncation that would drop one or more URLs from the end of pipe-separated list
            adapter['image_url'] = image_url[:2000]

        source_1 = adapter.get('source_1')
        if source_1 is None or len(source_1) == 0:
            raise DropItem("Constraint violation on item {}: source_1 cannot be empty".format(item))

        if type(source_1) == str and len(source_1) > 2048:
            raise DropItem("Constraint violation on item {}: source_1 cannot be longer than 2048 characters".format(item))
